...
public final class DataConverter {
	...
	public static DataObject toApi(Data pdu) {
		...
		case DATE_TIME:
			CosemDateTime dateTime = new CosemDateTime(pdu.date_time.getValue());
			return DataObject.newDateTimeData(dateTime);

		case DATE:
			CosemDate date = new CosemDate(pdu.date.getValue());
			return DataObject.newDateData(date);

		case TIME:
			CosemTime time = new CosemTime(pdu.time.getValue());
	...
	public static Data toPdu(DataObject data) {
		...
		if (choice == Choices.DONT_CARE || choice == Choices.NULL_DATA) {
			...
			result.setoctet_string(new AxdrOctetString(cal.ocletString()));
	...
	public static GetResult toApi(Get_Data_Result pdu) {
		...
		if (pdu.getChoiceIndex() == Get_Data_Result.Choices.DATA) {
			return new GetResult(toApi(pdu.data));
		}
		else {
	...
	public static EventNotification toApi(EVENT_NOTIFICATION_Request pdu) {
		...
		Long timestamp = null;
		if (pdu.time.isUsed()) {
			CosemDateTime dateTime = new CosemDateTime(pdu.time.getValue().getValue());
		...
		return new EventNotification(classId, obisCode, attributeId, newValue, timestamp);
	}
