...
abstract class ClientConnection implements TransportLayerConnectionListener {
	...
	void connect() throws IOException {
		...
		aarq.calling_AP_title = new AP_title(new BerOctetString(connectionSettings.systemTitle()));
	...
	private void setupAarqAuthentication(AARQ_apdu aarq, byte[] clientToServerChallenge) {
		aarq.mechanism_name = new BerObjectIdentifier(
	 ...
	 * 
	 * @see #get(boolean, GetRequestParameter...)
	 * 
	 * @param params
	 *            Varargs of specifiers which attributes to send (See {@link GetRequestParameter})
	 ...
	 */
	public List<GetResult> get(GetRequestParameter... params) throws IOException, TimeoutException {
	 ...
	 *             if the request times out
	 */
	public abstract List<GetResult> get(boolean highPriority, GetRequestParameter... params)
	 ...
	 * @param params
	 *            Varargs of specifier which attributes to set to which values (See {@link SetRequestParameter})
	 ...
	 *             throws IOException
	 */
	public List<AccessResultCode> set(long timeout, SetRequestParameter... params) throws IOException {
	 ...
	 *             throws IOException
	 */
	public List<AccessResultCode> set(SetRequestParameter... params) throws IOException {
		return set(3000, params);
	}
	 ...
	 *             throws IOException
	 */
	public abstract List<AccessResultCode> set(long timeout, boolean highPriority, SetRequestParameter... params)
			throws IOException;
	 ...
	 *            {@link MethodRequestParameter}
	 * @return List of results from the smart meter in the same order as the requests or null if confirmed has been set
	 *         to false on creation of this object
	 * @throws IOException
	 *             throws IOException
	 */
	public List<MethodResult> action(MethodRequestParameter... params) throws IOException {
	 ...
	 */
	public abstract List<MethodResult> action(boolean highPriority, MethodRequestParameter... params)
			throws IOException;
	...
	public void dataReceived(byte[] data) {
		...
		// TODO: remove
		// System.out.printf("%s#dataReceived(byte[]): data received!\n", getClass().getSimpleName());
	...
	protected abstract byte[] hlsAuthentication(byte[] processedChallenge, long timeout) throws IOException;
	...
	private void connectWithEnablededConfirmedMode(HlsSecretProcessor hlsSecretProcessor,
		...
		if (aare.result.val != 0) {
			...
			if (aare.result_source_diagnostic.acse_service_user != null) {
				errorMsg = "ACSE service user = " + aare.result_source_diagnostic.acse_service_user.val;
			}
			else {
				errorMsg = "ACSE service provider = " + aare.result_source_diagnostic.acse_service_provider.val;
			}
		...
		if (connectionSettings.authenticationMechanism().getCode() > 1) {
			...
			byte[] serverToClientChallenge = aare.responding_authentication_value.charstring.octetString;
			...
			try {
				remoteResponse = hlsAuthentication(processedChallenge, connectionSettings.responseTimeout());
			} catch (IOException e) {
			...
			processedChallenge = hlsSecretProcessor.process(clientToServerChallenge,
					connectionSettings.authenticationKey(), connectionSettings.globalEncryptionKey(),
					aare.responding_AP_title.ap_title_form2.octetString, frameCounterInt);
