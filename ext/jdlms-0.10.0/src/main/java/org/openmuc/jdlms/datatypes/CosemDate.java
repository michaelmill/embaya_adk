...
public class CosemDate implements CosemDateFormat {
	...
	public CosemDate(int year, int month, int dayOfMonth, int dayOfWeek) throws IllegalArgumentException {
		...
		this.ocletString[0] = (byte) ((year & 0xff00) >> 8);
		this.ocletString[1] = (byte) (year & 0xff);
		this.ocletString[2] = (byte) (month & 0xff);
		this.ocletString[3] = (byte) (dayOfMonth & 0xff);
		this.ocletString[4] = (byte) (dayOfWeek & 0xff);
	}

	public CosemDate(byte[] ocletString) {
		if (ocletString.length != SIZE) {
			throw new IllegalArgumentException("Wrong size.");
		}
		this.ocletString = ocletString;
	}
	...
	public int valueFor(Field field) {
		...
		case YEAR:
			...
			int year = this.ocletString[0] << 8;
			year |= this.ocletString[1] & 0xff;
			return year;

		case MONTH:
			return (this.ocletString[2] & 0xff) - 1;

		case DAY_OF_MONTH:
			return this.ocletString[3] & 0xff;

		case DAY_OF_WEEK:
			return this.ocletString[4] & 0xff;
