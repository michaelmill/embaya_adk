...
public class LnClientConnection extends ClientConnection {
	...
	@Override
	public List<GetResult> get(boolean highPriority, GetRequestParameter... params)
	...
	public List<AccessResultCode> set(long timeout, boolean highPriority, SetRequestParameter... params)
		...
		if (confirmedModeEnabled()) {
			...
			case SET_RESPONSE_NORMAL:
				result.add(AccessResultCode.forValue((int) response.set_response_normal.result.getValue()));
				break;

			case SET_RESPONSE_WITH_LIST:
				for (AxdrEnum res : response.set_response_with_list.result.list()) {
					result.add(AccessResultCode.forValue((int) res.getValue()));
				}
				break;
	...
	protected byte[] hlsAuthentication(byte[] processedChallenge, long timeout) throws IOException {
		...
		MethodRequestParameter authenticate = new MethodRequestParameter(15, new ObisCode(0, 0, 40, 0, 0, 255), 1,
				param);
	 ...
	 */
	private COSEMpdu createGetPdu(Invoke_Id_And_Priority id, GetRequestParameter... params) {
		...
		if (!ConformanceHelper.isAttribute0GetAllowed(negotiatedFeatures())) {
			for (GetRequestParameter param : params) {
		...
		if (params.length == 1) {
			...
			requestNormal.cosem_attribute_descriptor = new Cosem_Attribute_Descriptor(
					new Unsigned16(params[0].classId()), new Cosem_Object_Instance_Id(params[0].obisCode().getBytes()),
			...
			for (GetRequestParameter p : params) {
				...
				requestList.attribute_descriptor_list.add(new Cosem_Attribute_Descriptor_With_Selection(
						new Cosem_Attribute_Descriptor(new Unsigned16(p.classId()),
								new Cosem_Object_Instance_Id(p.obisCode().getBytes()), new Integer8(p.attributeId())),
						access));
			}
	...
	private List<COSEMpdu> createSetPdu(Invoke_Id_And_Priority id, SetRequestParameter[] params) throws IOException {
		if (params == null || params.length == 0) {
		...
		if (!ConformanceHelper.isAttribute0SetAllowed(negotiatedFeatures())) {
			for (SetRequestParameter param : params) {
				if (param.attributeId() == 0) {
		...
		if (params.length == 1) {
			...
			for (SetRequestParameter p : params) {
				Selective_Access_Descriptor access = null;
				SelectiveAccessDescription accessSelection = p.accessSelection();
				if (accessSelection != null) {
					access = new Selective_Access_Descriptor(new Unsigned8(accessSelection.accessSelector()),
				...
				Cosem_Attribute_Descriptor desc = new Cosem_Attribute_Descriptor(new Unsigned16(p.classId()),
						new Cosem_Object_Instance_Id(p.obisCode().getBytes()), new Integer8(p.attributeId()));
	...
	private List<COSEMpdu> createActionPdu(Invoke_Id_And_Priority id, MethodRequestParameter... params)
			throws IOException {
		if (params == null || params.length == 0) {
			throw new IllegalArgumentException("No parameter provided for set");
		}
		...
		if (params.length == 1) {
			...
			requestNormal.cosem_method_descriptor = new Cosem_Method_Descriptor(new Unsigned16(params[0].classId()),
					new Cosem_Object_Instance_Id(params[0].obisCode().getBytes()), new Integer8(params[0].methodId()));
			...
			for (MethodRequestParameter param : params) {
				Cosem_Method_Descriptor desc = new Cosem_Method_Descriptor(new Unsigned16(param.classId()),
						new Cosem_Object_Instance_Id(param.obisCode().getBytes()), new Integer8(param.methodId()));
	...
	protected void validateReferencingMethod() throws IOException {
		if ((negotiatedFeatures().bitString[2] & 0x1F) == 0) {
			disconnect(true);
