...
public class CosemTime implements CosemDateFormat {
	...
	public CosemTime(int hour, int minute, int second, int hundredths) throws IllegalArgumentException {
		...
		this.ocletString[0] = (byte) (hour & 0xff);
		this.ocletString[1] = (byte) (minute & 0xff);
		this.ocletString[2] = (byte) (second & 0xff);
		this.ocletString[3] = (byte) (hundredths & 0xff);
	}

	public CosemTime(byte[] ocletString) {
	...
	public int valueFor(Field field) {
		switch (field) {
		case HOUR:
			return this.ocletString[0] & 0xff;
		case MINUTE:
			return this.ocletString[1] & 0xff;
		case SECOND:
			return this.ocletString[2] & 0xff;
		case HUNDREDTHS:
