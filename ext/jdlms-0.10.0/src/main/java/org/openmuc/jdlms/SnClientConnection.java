...
import org.openmuc.jdlms.internal.InterfaceClass;
import org.openmuc.jdlms.internal.InterfaceClassList;
...
public class SnClientConnection extends ClientConnection {
	...
	// @Override
	public List<GetResult> get(long timeout, boolean highPriority, GetRequestParameter... params) throws IOException {
	...
	@Override
	public List<AccessResultCode> set(long timeout, boolean highPriority, SetRequestParameter... params)
			throws IOException {
		List<Variable_Access_Specification> shortNames = getVariableList(timeout, Arrays.asList(params));
		...
		if (confirmedModeEnabled()) {
			...
			try {
				response = writeResponseQueue.poll(timeout, TimeUnit.MILLISECONDS);
			} catch (InterruptedException e) {
				throw new IOException("Interrupted while waiting for incoming response", e);
			}
	...
	@Override
	public List<MethodResult> action(boolean highPriority, MethodRequestParameter... params) throws IOException {
		...
		if (confirmedModeEnabled()) {
			...
			for (MethodRequestParameter param : params) {
				Variable_Access_Specification access;

				ObjectInfo objectInfo = lnMapping.get(param.obisCode());
				InterfaceClass classInfo = InterfaceClassList.classInfoFor(objectInfo.classId, objectInfo.version);
	...
	private List<Variable_Access_Specification> getVariableList(long timeout, List<? extends AttributeRequest> params)
			throws IOException {
		...
		for (AttributeRequest param : params) {
			ObisCode obisCode = param.obisCode();
			...
			Integer16 variableName = new Integer16(info.baseName + 8 * (param.attributeId() - 1));
			if (param.accessSelection() == null) {
				...
				accessSpec.setparameterized_access(new Variable_Access_Specification.SubSeq_parameterized_access(
						variableName, new Integer8(param.accessSelection().accessSelector()),
						DataConverter.toPdu(param.accessSelection().accessParameter())));
			}
	...
	private ObjectInfo getVariableInfo(long timeout, AttributeRequest param) throws IOException {
		if (!ConformanceHelper.parameterizedAccessAllowedFor(negotiatedFeatures())) {
		...
		filter.structure.get(1).setoctet_string(
				new AxdrOctetString(new Cosem_Object_Instance_Id(param.obisCode().getBytes()).getValue()));
	...
	private List<EVENT_NOTIFICATION_Request> transformEventPdu(InformationReportRequest event) throws IOException {
		...
		outer: for (eventIter = event.variable_access_specification.iterator(), dataIter = event.list_of_data
			...
			for (ObisCode shortNameKey : lnMapping.keySet()) {
				...
				if (InterfaceClassList.classInfoFor(shortNameInfo.classId, shortNameInfo.version)
					...
					Cosem_Attribute_Descriptor logicalNameInfo = new Cosem_Attribute_Descriptor(
							new Unsigned16(shortNameInfo.classId),
							new Cosem_Object_Instance_Id(shortNameKey.getBytes()),
	...
	@Override
	protected byte[] hlsAuthentication(byte[] processedChallenge, long timeout) throws IOException {
		DataObject param = DataObject.newOctetStringData(processedChallenge);

		MethodRequestParameter authenticate = new MethodRequestParameter(12, new ObisCode(0, 0, 40, 0, 0, 255), 8,
				param);
	...
	protected void validateReferencingMethod() throws IOException {
		...
		if (negotiatedFeatures().bitString[0] == 0) {
			disconnect(true);
	...
	@Override
	public List<GetResult> get(boolean highPriority, GetRequestParameter... params) throws IOException {
